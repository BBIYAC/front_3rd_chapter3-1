...
describe('getEventsForDay', () => {
  it('월에 관계없이 특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    const testEvents = [...mockEvents];
    const events = getEventsForDay(testEvents, 1);
  ...
  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    const testEvents = [...mockEvents];
    const events = getEventsForDay(testEvents, 2);
...
describe('fillZero', () => {
  ...
  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    const filledZero = fillZero(30000, 2);

    expect(filledZero).toBe('30000');
  });
});

describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    const formattedDate = formatDate(new Date('2024-8-3'));

    expect(formattedDate).toBe('2024-08-03');
  });
