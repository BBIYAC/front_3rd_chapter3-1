...
it("view를 'week'으로 변경 시 'week'로 반영된다", () => {
  const { result } = renderHook(() => useCalendarView());
...
it("주간 뷰에서 이전으로 navigate 시 7일 전 '2024-09-24' 날짜로 지정이 된다", () => {
  const { result } = renderHook(() => useCalendarView());
...
it("월간 뷰에서 다음으로 navigate 시 한 달 후 '2024-11-01' 날짜여야 한다", () => {
  const { result } = renderHook(() => useCalendarView());
...
it("월간 뷰에서 이전으로 navigate 시 한 달 전 '2024-09-01' 날짜여야 한다", () => {
  const { result } = renderHook(() => useCalendarView());
